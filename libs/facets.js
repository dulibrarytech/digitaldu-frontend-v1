...
function createBreadcrumbTrail(data, dates, baseUrl) {
    ...
    for (var i = 0; i < data.length; i++) {
        html += '<span><a onclick="removeFacet(\'' + data[i].type + '\', \'' + data[i].facet + '\', \'' + baseUrl + '\')"><strong style="color: red">X</strong></a>&nbsp&nbsp' + data[i].type + '&nbsp&nbsp<strong style="color: green"> > </strong>&nbsp&nbsp' + data[i].name + '</span>';   // good
    }

    for (i = 0; i < dates.length; i++) {
        html += '<span><a onclick="removeDateRange(\'' + dates[i].from + '\', \'' + dates[i].to + '\')"><strong style="color: red">X</strong></a>&nbsp&nbspDate Range&nbsp&nbsp<strong style="color: green"> > </strong>&nbsp&nbsp' + dates[i].from + ' - ' + dates[i].to + '</span>';   // good
    }
    return html;
};
